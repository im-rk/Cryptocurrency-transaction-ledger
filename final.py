...
class TransactionLedger(BlockchainComponent):
    ...
    def verify_chain(self):
        transactions = list(collection.find().sort("timestamp", 1))  # Get all transactions sorted by time
        print(f"DEBUG: Retrieved Transactions -> {transactions}") 
        if not transactions:  # If the transactions list is empty
            return {"message": "Ledger is empty.", "valid": True}  # Empty ledger is technically valid
        ...
        for i in range(1, len(transactions)):
            recalculated_hash = self.calculate_hash(
                ...
                str(transactions[i]["timestamp"]),  # Ensure timestamp is a string
                transactions[i - 1]["hash"]  # Use previous transaction's hash
            )
    ...
    def display_ledger(self):
            ...
            for txn in transactions:
                ...
                timestamp_str = txn['timestamp'].strftime('%Y-%m-%d %H:%M:%S')
